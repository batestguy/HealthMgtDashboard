...
- **Cache-bust local assets.** `index.html` appends `?v=N` to every local `css/` and `js/` reference (currently `?v=14`). Bump N on every app change — Pages caches ~10 minutes and a stale mix of old/new files is the usual "it works locally" failure.
...
- **Chart.js sizes the backing store from the canvas's _parent_, not the canvas.** Any wrapper that can grow independently of its canvas silently desynchronises the two: Chart.js allocates a surface for the parent's height, CSS squashes the element back down, and the chart renders vertically compressed with pointer hit-testing drifted by the same margin — while looking completely correct in a screenshot. **After changing any chart wrapper, assert `canvas.height === canvasCssHeight * devicePixelRatio`**; the visual check alone passes on a broken layout. This is why `#tab-health .chart-wrap-tall` uses a fixed `height: calc(280px + var(--hl-band))` with `padding-bottom: var(--hl-band)` and an absolutely-positioned `.hl-legend`, rather than the column flex (`height: auto` + `canvas { flex: 0 0 280px }`) that reads as the obvious fix — the band keeps the wrapper's *content* box at exactly 280px while still giving the bespoke legend somewhere to live. `--hl-band` is 64px, sized for the two-line wrap **two** series reach at 375px; a third series needs a taller band.
