...
- **Feature set (5 tabs, fixed bottom tab bar):**
  ...
  3. 🧠 **Quiz** – 10–12 questions per session (5 static Nigeria trivia + 5–7 dynamic from uploaded Excel), one at a time in batches of 3, instant feedback, running score, results screen with review.
...
- Shipped app files (spec §3.1): `index.html` (5-tab shell), `css/styles.css`, `js/app.js` (tabs + Projects tab), `js/data.js` (Excel parse/validate), `js/charts.js` (bar/doughnut/line), `js/health-data.js` (GRID3 + HDX + seeds), `js/map.js` (Leaflet), `js/health.js` (Health tab), `js/quiz.js` (Quiz tab: static + dynamic pool, one-at-a-time flow, results review). `js/nlq.js`, `js/export.js` are pending.
