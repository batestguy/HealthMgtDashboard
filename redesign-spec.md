...
2. **Export tab — real first cut (PNG + share link now; PDF now live).** PNG snapshot of the active tab via `html2canvas` (retina-scaled, themed to the active light/dark theme, filename `dashboard-YYYY-MM-DD.png`). Share link that copies the live Pages URL + active tab hash (clipboard API with `execCommand` fallback, toast feedback). **PDF is now live too:** the "Export as PDF" card wires `js/pdf.js`, which is the multi-section report built with **jsPDF loaded from the CDN** (`https://cdn.jsdelivr.net/npm/jspdf@4.2.1/dist/jspdf.umd.min.js?v=10` in the `<head>` library block). Sections: (1) title page, (2) Projects KPIs + task summary + per-project budget/spend + project-progress bar chart, (3) Health KPIs + facilities-by-level doughnut + indicator-trends line chart, (4) latest Ask answer. Charts are embedded as JPEG snapshots from the Chart.js instances, re-rendered from live/cached data before capture. Always prints in the light "paper" theme regardless of active UI theme (print = paper). Clicking "Download PDF report" flips the deferred-PDF card to live, shows "Building PDF report…", then downloads `dashboard-report-YYYY-MM-DD.pdf`; if jsPDF's CDN fails to load, the button reports that on the status line and the card still falls back gracefully. The PNG + share link remain the earlier Export first cut; the bespoke Health trend legend is untouched.
...
5. **Export** — **PNG snapshot** (active-tab `html2canvas`, retina, themed to active theme, `dashboard-YYYY-MM-DD.png`) **+ share link** (live Pages URL + active tab hash, clipboard + `execCommand` fallback, toast) **now live**; **PDF now live** — multi-section report built with **jsPDF loaded from the CDN** (`jspdf@4.2.1` UMD build in the `<head>` library block), sections: title page → Projects KPIs + task summary + project-progress bar chart → Health KPIs + facilities-by-level doughnut + indicator-trends line chart → latest Ask answer; charts embedded as JPEG snapshots from the Chart.js instances, re-rendered before capture; always green/gold on paper regardless of active theme; filename `dashboard-report-YYYY-MM-DD.pdf`; "Download PDF report" flips the card to live and shows "Building PDF report…" → "PDF downloaded" (or "PDF library did not load from CDN" + graceful fallback if the CDN fails).
...
- When a JS module assumes a global library (here `jsPDF`), the library must actually be loaded on the page — adding the module file alone is not enough; `index.html` must include the CDN `<script>` for jsPDF (done: `jspdf@4.2.1` UMD build in the `<head>` library block). This is the lesson behind the one-line fix that made the PDF button work.

**Current build:** `?v=10` (10 js files + css/styles.css; jsPDF now loaded from the CDN in the `<head>` library block at `jspdf@4.2.1`) on GitHub Pages → https://batestguy.github.io/HealthMgtDashboard/.

**All functional items are now delivered.** The multi-section PDF report (`jsPDF`) — the one remaining open item — is now live: `js/pdf.js` (committed), jsPDF loaded from the CDN in `index.html` (`jspdf@4.2.1` UMD build in the `<head>` library block), multi-section report (title page → Projects KPIs + task summary + project-progress bar chart → Health KPIs + facilities-by-level doughnut + indicator-trends line chart → latest Ask answer), charts embedded as JPEG snapshots from the Chart.js instances, always green/gold on paper regardless of active theme, filename `dashboard-report-YYYY-MM-DD.pdf`. The bespoke Health trend legend is untouched.
