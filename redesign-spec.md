...
- **Chart wrapper layout — assert the backing store (commited 2026-09-08):** Chart.js sizes a canvas's backing store from the canvas's **parent element**, not from the canvas's own CSS box, so any wrapper that can grow independently of the canvas silently desynchronises the two. The Health trend wrapper (`#tab-health .chart-wrap-tall`) needed to grow, because the bespoke HTML legend from the bullet above lives *below* the canvas and at 375px it wraps to two lines and overprinted the map-credit line. The obvious fix — make the wrapper a column flex (`height: auto; min-height: 280px`) and pin the canvas with `flex: 0 0 280px` — passed every visual check: legend inside the wrapper, zero collisions, canvas measuring 280px CSS, stable across six resizes. It was still wrong. The wrapper had grown to 347px, so Chart.js allocated a **1041px** backing store (347 × `devicePixelRatio` 3) while flex-basis squashed the element back into 280px: the chart drew ~19% vertically compressed with pointer hit-testing drifted by the same margin, and `chartArea.height` read 312 instead of 245. Nothing about that is visible in a screenshot. What shipped instead keeps the wrapper's *content* box at exactly 280px and reserves the legend a band outside it — `height: calc(280px + var(--hl-band)); padding-bottom: var(--hl-band)` with the legend `position: absolute; top: calc(100% - var(--hl-band))` (`top`, not `bottom: 0`, so it stays flush under the canvas on desktop where it is one line). The rule going forward: **when you change any chart wrapper's layout, assert `canvas.height === canvasCssHeight * devicePixelRatio`** — the visual check alone passes on a broken layout. Known limit, recorded honestly: `--hl-band` is 64px, sized for the two-line wrap the *two*-series legend reaches at 375px (59px measured); a third series would need a taller band. A `@media (min-width: 900px)` override to reclaim the ~39px of desktop slack was considered and rejected — between 900px and ~1100px the `.health-split` two-column layout can still wrap the legend to two lines and reintroduce the overprint.
...
2. **Health** — GRID3 facility map (Nigeria-framed, zoom-8 detail, marker clusters, atlas-styled circles + Leaflet chrome + bespoke state-tooltip), HDX indicator trends + facility-level doughnut + key indicators; KPIs with mono numerals + footer summary; atlas section rules. Restyled in Phase 2. The trend wrapper reserves a fixed `--hl-band` below its 280px canvas box and absolutely positions the bespoke legend into it — see §4.6 for why the band is fixed rather than an auto-growing flex column.
...
- Chart.js sizes the backing store from the canvas's **parent**, so a wrapper that grows independently of the canvas renders the chart compressed and drifts pointer hit-testing while looking correct in a screenshot. Assert `canvas.height === canvasCssHeight * devicePixelRatio` after any chart-wrapper change. See the 2026-09-08 lesson in §4.6.

**Current build:** `?v=14` (10 js files + css/styles.css; jsPDF `4.2.1` + html2canvas `1.4.1` loaded from the CDN in the `<head>` library block) on GitHub Pages → https://batestguy.github.io/HealthMgtDashboard/.
...
- 2026-09-08 — **Health legend overprint fixed; the 375×812 touch-target row finally passes.** Two long-standing `dashboard-spec.md` §9 failures closed, both CSS-only; no `js/*.js` file was touched. (1) The bespoke Health trend legend sat outside its wrapper and printed on top of the map-credit line at narrow widths — at 375px the two-series legend wraps to two lines (59px measured) and `#tab-health .chart-wrap-tall` was a flat `height: 280px`. Fixed by reserving a `--hl-band: 64px` band (`height: calc(280px + var(--hl-band)); padding-bottom: var(--hl-band)`) and absolutely positioning `.hl-legend` into it at `top: calc(100% - var(--hl-band))`. Measured after: legend 2322–2381 with its top exactly on the canvas bottom, credit 2417–2448, zero collisions in light and dark; at 1280×900 the legend is one 25px line with 39px of slack below it. **The obvious fix was wrong and is the reason this entry exists** — a column flex with `height: auto` and `canvas { flex: 0 0 280px }` passed every visual check while Chart.js, which sizes its backing store from the canvas's *parent*, allocated 1041px (347 × DPR 3) against an expected 840 and drew the chart ~19% compressed with hit-testing drifted to match (`chartArea.height` 312 vs 245). See the 2026-09-08 bullet in §4.6; the standing rule is to assert `canvas.height === canvasCssHeight * devicePixelRatio` on any chart-wrapper change. (2) Touch targets at 375×812 went from 30 sub-44px elements to 7: `.btn-sm` and `.chip` 36→44 (`min-height`, since "small" is type scale and padding, not hit area), `.icon-btn` 40→44, `.brand` 39→44, Leaflet's own zoom buttons 30→44 (third-party chrome held to the same floor rather than left as the one exception), and `.indicator-row`'s vertical padding moved onto `.indicator-label` so the full-width label *is* the 44px target at an unchanged row height. The 24px checkbox cannot be padded to 44 without drawing a giant box, so `.indicator-label::before { inset: 0 }` stretches an invisible overlay across the whole row — the label already owns the checkbox via `for`, so `js/health.js` stays untouched and a click still fires exactly one `change`. Verified with `elementFromPoint` and clicks at 0.03, 0.5 and 0.98 of the row width; every hit resolved to `label.indicator-label` and toggled. The 7 remaining are those 5 covered checkboxes plus the two Leaflet/OSM attribution links (43×11, 71×11), exempt under the WCAG 2.5.8 inline exception for links inside a sentence of text. Zero horizontal overflow on all five tabs, zero console errors or warnings across the walk, and the trend legend still paints DPT3 `rgb(46,174,125)` green against Malaria `rgb(243,196,90)` gold with `legend: { display: false }` intact — `healthTrendColors()` and `renderHealthLegend()` were not modified. `?v=13` → `?v=14`.
