...
- Bars: per-bar gradient fill from the bar's own color at the axis to ~20% alpha at the tip (`gradient()` in `js/charts.js`), `borderRadius` 6, `maxBarThickness` kept; the **leading (highest) bar** switches to a gold gradient. Hover reverts to the flat color. `hbar()` uses the same helper on the horizontal axis; `groupedBar()` gradients each dataset in its own color.
